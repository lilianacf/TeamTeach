...
if ((isset($_POST["guardar"]))&&($_POST["guardar"]=="frmmas"))
{
    ...
    elseif ($_POST["codmasc"]=="" || $_POST["nommasc"]== "" || $_POST["color"]==""
    || $_POST["raza"]==""){
        ...
        <table border="1"class="center" >
            <form name="frm_mas" method="POST" autocomplete="off">
              ...
              <tr>
                  <th>Tipo Mascota</th>
                  <th>
                  <select name ="tip_masc")
                    <option value="">Seleccione el tipo de mascota</option>
                    <?php
                    $sql_tmasc= "Select * from tipo_mascotas";
                    $query_tmasc=mysqli_query($mysqli, $sql_tmasc);
                    $fila=mysqli_fetch_assoc($query_tmasc);
                        //Codigo php ciclo
                        do{
              
                    ?>
                    <option value="<?php echo ($fila['tip_masc'])?>"> <?php echo ($fila['tip_masc'])?></option>
                    <?php   } while($fila=mysqli_fetch_assoc($query_tmasc));
                    ?>
